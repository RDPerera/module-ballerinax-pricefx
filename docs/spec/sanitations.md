...
576. Accept a caller-supplied Pricefx JWT (`jwt`)
- **Gap**: item 575 kept `generateJwtToken` / `generateTimedJwtToken` exposed, and Pricefx describes the former as producing a **non-expiring** JWT "for integration purposes". But there was no way to then *use* such a token: `ConnectionConfig` accepted username/password, `pricefxKey`, `oauth2*` or `externalJwt`, and none of those send a caller-supplied token as `X-PriceFx-jwt`. `externalJwt` is not a substitute - it formats the value as `Authorization: BEARER <system>;<jwt>`, which is the External-JWT trust scheme, not a Pricefx-issued session token.
- **Added**: a `jwt` field on `PricefxCredentials`, taking the highest precedence in `createOasClient`. It maps straight to `oas:ApiKeysConfig`, so the token is sent verbatim as `X-PriceFx-jwt` and **no exchange happens at all** - constructing a client on this path performs no network request, unlike the `pricefxKey` path which calls `POST /token` during `init()`.
- **Deliberately not done - minting these tokens automatically**: it would have been possible to have the connector call `generateJwtToken` itself during `init()` (it only needs Basic auth, so there is no circularity) and thereby never deal with expiry. That is the wrong default: every `init()` would mint a *new permanent* credential, so an application restarted N times leaves N immortal tokens on the Pricefx side with no cleanup path, and a long-lived credential held in memory has a much worse blast radius if leaked than a ~30-minute session token. Pricefx's own framing ("used for integration purposes") describes a token provisioned once, deliberately, and stored in configuration - which is exactly what the `jwt` field consumes. The default bootstrap therefore stays on the short-lived `POST /token` session token.
- **Known limitation, documented in the field's own docs**: a `jwt` supplied this way cannot be refreshed - the connector holds no credential to re-authenticate with, so a 401 surfaces to the caller instead of triggering the usual retry. Correct for a non-expiring token; wrong if someone pastes in a session token, hence the explicit warning.

577. Build warnings: what was fixed, and what is inherent
- **Starting point**: `./gradlew build` succeeded but emitted ~4,650 warnings per compilation, which is enough noise to hide a real problem. They break down into three groups.
- **Fixed - the deprecated `export` declaration (1)**: `export = [...]` under `[package]` is deprecated in favour of an `export` field under `[[package.modules]]`. Changed in both `ballerina/Ballerina.toml` **and** `build-config/resources/Ballerina.toml` - that second file is the template the `updateTomlFiles` gradle task copies over `Ballerina.toml` on every build, so a change made only to the live file is silently reverted the next time gradle runs. (This is what had quietly dropped the `export` line earlier.) Verified after the change that a consumer can still name `oas:` types.
- **Fixed - doc comments in hand-written code (359)**: 351 "undocumented parameter" in the wrapper's `client.bal`, 7 undocumented fields on the hand-written token-exchange types, and 1 stale parameter name in the test mock. The wrapper ones are generated by the build script, so the fix went into the generator: it now reconciles each copied doc comment against the real signature - adding a `+ <name> -` line for any parameter the generated docs omitted (usually `payload`) and dropping lines documenting a `request` parameter that only exists inside the generated function body. Care is needed there: `+ return -` matches the same "parameter doc" pattern and must be excluded, or every return doc gets stripped.
- **Not fixable - undocumented fields in generated code (~4,290)**: 3,940 in `modules/oas/types.bal` plus 349 in `modules/oas/client.bal`. These are not a tool defect: **97% of the schema properties in Pricefx's own spec carry no `description`** (386 of 13,384 do). `bal openapi` emits a doc comment only where the spec supplies one, so the warnings are an upstream documentation gap. Writing filler descriptions for 13,000 fields would be worse than leaving them undocumented - it would manufacture text that reads as documentation while carrying no information - and it would mean hand-editing generated code, which item 574's regeneration guarantee forbids. Left as-is deliberately.
- **Not fixable - 2 deprecation warnings**: the wrapper forwards `changeTermsOfUse`, which Pricefx marks deprecated. The wrapper's own function carries `@deprecated` too, so callers are warned correctly, but Ballerina still warns at the call site inside the forwarder. Removing the operation would be worse (it is a valid, if deprecated, spec operation); the warning is accurate.

578. Keep `lock` out of the wrapper client class (Swan Lake Update 13.x codegen bug)
- **Symptom**: the connector compiled and tested cleanly on the pinned 2201.12.0, but on 2201.13.4 every operation failed at run time with `java.lang.VerifyError: Bad type on operand stack`, pointing at `ballerinax/pricefx/0/values/$Client$split$1.getOasClient` and complaining that a `Strand` was on the stack where a `$Client` was expected. Compilation gave no error - only the JVM's own bytecode verifier rejected the class, at first use.
- **Cause**: the wrapper's `Client` carries 477 remote methods, which is large enough that the compiler splits it across several JVM classes (`$Client$split$N`). On Update 13.x, a `lock` statement inside a *split* class emits invalid bytecode. Update 12 compiles the same source correctly, so this is a toolchain regression rather than invalid Ballerina - but the connector has to work on current releases, not just the pinned one. Rewriting `getOasClient` to read into a local and return outside the lock (instead of `return self.oasClient;` inside it) did **not** help: the trigger is the presence of `lock` in a split class, not the `return` placement.
- **Fix**: moved the mutable state and its `lock` into a small separate `OasClientHolder` class, which is far too small to be split. `Client` now holds `private final OasClientHolder holder` and contains **no `lock` at all** - every one of its fields is `final`, so it needs none. `getOasClient()` delegates to `holder.get()` and `reauthenticate()` to `holder.set(...)`. Verified working on both 2201.12.0 and 2201.13.4.
- **Why this was missed**: all verification had been done through the pinned 2201.12.0 container, where the bug does not reproduce. A connector that only works on the distribution it pins is not much use, so the auth examples are now run with the developer's own `bal` by default rather than through the container.
- **Note for future edits**: do not reintroduce a `lock` (or any construct that needs one, i.e. a non-`final`, non-`readonly` field) into `Client` while it stays this large. Put the state in a small helper class instead.

579. Basic auth: obtain a session token once, and drop the `pricefxKey` path
- **The defect**: the Basic auth path configured `http:CredentialsConfig`, which attaches `Authorization: Basic ...` to **every** request. Pricefx states that "the basic authentication leads to an extra 500 ms request execution time as the password verification is intentionally slow to mitigate brute-force password guess attacks", and that it issues a JWT "as a cookie ... when an API call with Basic authentication (user/pwd) is called", recommending clients "use the JWT for subsequent calls (and not user/pwd on every API call)". The connector did neither: it re-sent credentials each time and discarded the cookie (Ballerina's `CookieConfig.enabled` defaults to `false` and was never turned on). Every call therefore paid the deliberate ~500ms penalty - 100 calls meant ~50 seconds of artificial delay. Mock testing could never catch this: the requests were correctly *formed*, just needlessly slow.
- **The fix**: `bootstrapSessionJwt()` makes exactly one Basic authenticated `GET /login/extended`, reads the `X-PriceFx-jwt` value out of the response's `Set-Cookie` header, and the client then authenticates with `oas:ApiKeysConfig` for the rest of its life. The penalty is paid once per client instead of once per request. Cookie support is deliberately *not* enabled on the client - the header is parsed directly, which keeps the behaviour explicit and avoids relying on cookie-jar semantics we would not otherwise use. Token expiry is already handled: the existing 401 retry rebuilds the client, which re-bootstraps.
- **Fails soft, on purpose**: `bootstrapSessionJwt` returns `string?` and never propagates an error. A network failure, a deployment that does not serve that endpoint, or a response without the cookie all yield `()`, and the caller falls back to per-request Basic auth - slower, but exactly the previous behaviour. This matters because the cookie mechanism could not be verified against a live Pricefx partition; turning an unverifiable optimization into a hard initialization failure would have been the wrong trade. Both paths are covered by tests (`testBasicAuthBootstrapsASessionToken`, `testFallsBackToBasicAuthWhenNoSessionCookieIsIssued`), the latter against a second mock service that authenticates but issues no cookie.
- **`pricefxKey` removed**: it exchanged credentials plus an API key at `POST /token` for a session token. Once Basic auth bootstraps a session token by itself, that path only saved the one-off ~500ms on the bootstrap call, while adding a distinct credential, a second bootstrap mechanism, and its own request/response types. The two remaining credential-based options now cover the space cleanly: username/password when you want the connector to obtain and renew a session token, or `jwt` when you hold a long-lived integration token. `fetchAccessToken`, `TokenExchangeRequest` and `TokenExchangeResponse` went with it.
- **Testing note**: `run_all.sh` was silently running stale example binaries - it did not clear `target/`, so a rebuilt connector appeared to change nothing. It now clears each example's build output before running. That is what initially made this change look like it only worked for some examples.

580. Make credentials a typed union, and drop CSRF support
- **Credentials were 11 loose optional strings** on one closed record, so `{username: "u"}` alone or `{jwt: "t", username: "u"}` both compiled and only failed at run time, and precedence silently decided which of a conflicting pair won. Replaced with a union of four records - `BasicCredentials`, `JwtCredentials`, `OAuth2Credentials`, `ExternalJwtCredentials` - exposed as `PricefxCredentials`. Because each is a *closed* record with required fields, a record literal matches exactly one member, so the compiler now rejects both examples above with `incompatible mapping constructor expression for type 'PricefxCredentials'`. Verified.
- **Where it lives**: `PricefxCredentials` is a field on `ConnectionConfig` (`auth`), so the client is constructed as `check new ({auth: {username, password, partition}}, serviceUrl)` and `init` keeps its conventional single-config signature. It was briefly `init`'s first parameter instead, to preserve the flatter `check new ({username, password, partition}, ...)` form, but nesting it under `auth` matches both the generated `oas:ConnectionConfig` and the rest of the `ballerinax` connectors, and it type-checks strictly better: a config with no `auth` at all is now rejected too (`missing non-defaultable required record field 'auth'`), which the parameter form could not catch because the union has no empty member. Being a union rather than a record, it has to be a field - `*PricefxCredentials` inclusion only works for records.
- **CSRF support removed** on request. With TFA already gone (item 573's field was dropped earlier for the same reason - a code that expires in ~30 seconds cannot live in configuration), CSRF was the last thing needing a static per-request header. Both are now passed per call, via the `headers` argument every operation already takes.
- **Which let the whole header-merging layer go**: `ExternalJwtCredentials` moved into `oas:ConnectionConfig.auth` as an `http:BearerTokenConfig` carrying Pricefx's `<systemName>;<jwt>` value, so nothing needs injecting into requests any more. `buildStaticHeaders`, `mergeHeaders`, `staticHeaders()` and the per-function `mergedHeaders` line in all 477 forwarders are gone; each forwarder is now a plain pass-through again. **One behavioural note**: the scheme now goes out as `Bearer` where Pricefx's docs write `BEARER`. RFC 7235 defines the auth scheme as case-insensitive so this should be equivalent, but it is unverified against a live partition - if external JWT auth ever fails there, this is the first thing to check.

581. Give the response types idiomatic names
- **Problem**: 21 type names reaching the public API were not PascalCase, because they came straight from the spec's `components/responses` keys - `generalResponse`, `productResponse`, `quoteResponse`, `manualpricelistResponse`, `loaddataResponse`, and so on. Ballerina types are PascalCase by convention, and `generalResponse` in particular said nothing about what it holds.
- **Fixed at the spec, not in the generated code**: renamed the 16 `components/responses` keys, their inline `schema.title` values (which is what actually drives the generated type name), and all 75 `$ref`s pointing at them. `generalResponse` became `GenericDataResponse`, which describes what it is - Pricefx's standard envelope whose `data` property is untyped and nullable, shared by 16 operations. The rest just gained proper casing and word breaks (`manualpricelistResponse` -> `ManualPriceListResponse`, `loaddataResponse` -> `LoadDataResponse`).
- Also gave explicit `title`s to the five inline request/response bodies of the catch-all operations restored in item 568, which `bal openapi` had otherwise auto-named `fetch_typeCode_body`, `createObjectRequest` and similar; they are now `ListObjectsRequest`, `CreateObjectRequest`, `DeleteObjectRequest`, `GetObjectResponse`, `DeleteObjectResponse`.
- Checked for collisions against every already-generated type and every other spec component before applying; there were none. After regeneration no lowercase-leading type appears anywhere in the connector's public API.

582. Re-check every auth mechanism against the live Pricefx documentation
- Compared the connector against https://api.pricefx.com/rest-api/authentication mechanism by mechanism. Aligned as documented: the Basic credential is `<partition>/<username>:<password>`; the session token is read from the `X-PriceFx-jwt` cookie returned by `GET /login/extended`; the OAuth2 refresh URL resolves to the documented `POST /pricefx/{partition}/oauth/token`; the TFA (`PriceFx-TFA`) and CSRF (`X-PriceFx-Csrf-Token`) header names match and are available per call; and `generateJwtToken` (`/accountmanager.getjsonwebtoken`), the documented source of non-expiring integration tokens, is still exposed.
- **One genuine mismatch found and fixed - the external JWT scheme casing.** Item 580 had moved `ExternalJwtCredentials` onto `http:BearerTokenConfig`, which emits `Authorization: Bearer <value>`. Pricefx documents this mechanism verbatim as `Authorization: BEARER <externalSystemName>;<signed JWT token>` - in capitals. RFC 7235 does define the scheme as case-insensitive, and that reasoning was used to accept the deviation at the time, but it was the wrong call for two reasons: Pricefx documents mixed-case `Bearer` for OAuth 2.0 access tokens in the *same* page while writing this one in capitals, which suggests the external-JWT path is parsed literally rather than through standard bearer handling; and the `<system>;<jwt>` value is not a standard bearer token at all, so there is no reason to assume standard parsing. Demonstrated concretely by making the mock match the scheme case-sensitively: the connector then failed with a 401, its `Bearer` header falling through to be misread as an OAuth2 token. The header is now written directly as `BEARER ...` by `buildStaticHeaders`, which is also why the per-request header merge exists again in the 477 forwarders after item 580 had removed it.
- **A comment that was wrong, now corrected.** The external-JWT path sets `oas:ApiKeysConfig{X\-PriceFx\-jwt: ""}` as a placeholder, since `ConnectionConfig.auth` is required and `ApiKeysConfig` is the only union member that does not overwrite `Authorization`. A code comment claimed the empty value was "dropped rather than sent". Probing the wire with the Python mock disproved that - the header **is** sent, empty. An earlier probe had suggested otherwise, but that went through Ballerina's own header binding, which reports an empty header as absent; the Python server sees the real bytes. Suppressing it would require editing generated code, so the comment now states the actual behaviour and flags that Pricefx ignoring an empty session token alongside a valid `Authorization` header is expected but unverified.
- **Still unverified against a live partition** (unchanged): whether the Basic-auth session cookie behaves as documented (this fails soft, falling back to per-request Basic auth), and whether CSRF applies to header-authenticated requests, which their documentation contradicts itself on.
