...
- **The call site stays flat**: credentials moved out of `ConnectionConfig` and became `init`'s first parameter - `init(PricefxCredentials auth, string serviceUrl = ..., ConnectionConfig config = {})`. So `check new ({username, password, partition}, serviceUrl)` still reads the same as before, while now being type-checked; HTTP transport settings move to the optional third argument. This keeps the ergonomics that motivated flattening the record in the first place *and* gets the static discrimination that flattening had cost.
