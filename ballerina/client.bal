...
# top: exchanging `auth` for a short-lived JWT (or using it directly as HTTP Basic auth), and
# transparently re-authenticating and retrying once whenever a request comes back unauthenticated
# - a JWT is valid for around 30 minutes - so a long-lived client instance keeps working without
...
public isolated client class Client {
    ...
    remote isolated function addManualPriceListProducts(string id, oas:AddProductsToManualPriceListRequest payload, map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->addManualPriceListProducts(id, payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function pingWithout(map<string|string[]> headers = {}) returns error? {
        oas:Client oasClient = self.getOasClient();
        error? r = oasClient->pingWithout(headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function cancelCfsCalculation(string id, map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->cancelCfsCalculation(id, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function listTasks(map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->listTasks(headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function deleteAuthToken(oas:DeleteAuthTokenHeaders headers = {}) returns error? {
        oas:Client oasClient = self.getOasClient();
        error? r = oasClient->deleteAuthToken(headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function deleteCustomForm(oas:DeleteCustomFormRequest payload, map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->deleteCustomForm(payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function revokeCompensationRecord(string typedId, map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->revokeCompensationRecord(typedId, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function createKvTable(string tableName, oas:CreateKVTableRequest payload, map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->createKvTable(tableName, payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function insertBulkKvData(string tableName, oas:InsertBulkKVDataRequest payload, map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->insertBulkKvData(tableName, payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    # + queries - Queries to be sent with the request 
    # + return - OK 
    remote isolated function downloadFileViaPost(string typedId, string binaryDataId, map<string|string[]> headers = {}, *oas:DownloadFileViaPostQueries queries) returns oas:FileOperationEnvelope|error {
        oas:Client oasClient = self.getOasClient();
        oas:FileOperationEnvelope|error r = oasClient->downloadFileViaPost(typedId, binaryDataId, headers, queries = queries);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function deleteFile(string typedId, string binaryDataId, record {} payload, map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->deleteFile(typedId, binaryDataId, payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function sendValidationMessage(oas:NotificationSendBody payload, map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->sendValidationMessage(payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function markAsRead(record {} payload, map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->markAsRead(payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function sendEmail(oas:SendEmailRequest payload, map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->sendEmail(payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function login(map<string|string[]> headers = {}) returns oas:UserLoginResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:UserLoginResponse|error r = oasClient->login(headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
            r = oasClient->login(headers);
        }
        return r;
    }
    ...
    remote isolated function cancelJob(string id, record {} payload, map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->cancelJob(id, payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function uploadFileToPxCxSx("PX"|"CX"|"SX" typeCode, string target, string uploadSlotId, oas:TargetuploadSlotIdBody payload, map<string|string[]> headers = {}, *oas:UploadFileToPxCxSxQueries queries) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->uploadFileToPxCxSx(typeCode, target, uploadSlotId, payload, headers, queries = queries);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    # # Deprecated
    @deprecated
    remote isolated function changeTermsOfUse(oas:AccountmanagerChangetermsofuseBody payload, map<string|string[]> headers = {}) returns error? {
        oas:Client oasClient = self.getOasClient();
        error? r = oasClient->changeTermsOfUse(payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function deleteInternationalizationMessages(oas:I18nmanagerDeleteKeysBody payload, map<string|string[]> headers = {}) returns error? {
        oas:Client oasClient = self.getOasClient();
        error? r = oasClient->deleteInternationalizationMessages(payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function samlSignOn(map<string|string[]> headers = {}, *oas:SamlSignOnQueries queries) returns error? {
        oas:Client oasClient = self.getOasClient();
        error? r = oasClient->samlSignOn(headers, queries = queries);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
            r = oasClient->samlSignOn(headers, queries = queries);
        }
        return r;
    }
    ...
    remote isolated function deleteImportChanges(oas:ImportmanagerDeletechangesBody payload, map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->deleteImportChanges(payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function updateReviewStatus(string typedId, record {} payload, map<string|string[]> headers = {}) returns oas:generalResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:generalResponse|error r = oasClient->updateReviewStatus(typedId, payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function setReviewAsDone(string typedId, record {} payload, map<string|string[]> headers = {}) returns error? {
        oas:Client oasClient = self.getOasClient();
        error? r = oasClient->setReviewAsDone(typedId, payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    # + headers - Headers to be sent with the request 
    # + return - OK 
    remote isolated function fetchPendingReviews(map<string|string[]> headers = {}) returns error? {
        oas:Client oasClient = self.getOasClient();
        error? r = oasClient->fetchPendingReviews(headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function loadDataIntoFieldCollection(string typedId, oas:DatamartLoadfctypedIdBody payload, map<string|string[]> headers = {}) returns error? {
        oas:Client oasClient = self.getOasClient();
        error? r = oasClient->loadDataIntoFieldCollection(typedId, payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    remote isolated function oauthAuthorize(map<string|string[]> headers = {}, *oas:OauthAuthorizeQueries queries) returns error? {
        oas:Client oasClient = self.getOasClient();
        error? r = oasClient->oauthAuthorize(headers, queries = queries);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
            r = oasClient->oauthAuthorize(headers, queries = queries);
        }
        return r;
    }
    ...
    remote isolated function oauthToken(map<string|string[]> headers = {}, *oas:OauthTokenQueries queries) returns oas:OAuthTokenResponse|error {
        oas:Client oasClient = self.getOasClient();
        oas:OAuthTokenResponse|error r = oasClient->oauthToken(headers, queries = queries);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
            r = oasClient->oauthToken(headers, queries = queries);
        }
        return r;
    }
    ...
    # + headers - Headers to be sent with the request 
    # + return - OK 
    remote isolated function executeLibraryFunction(string formulaName, string elementName, string functionName, oas:ElementNamefunctionNameBody payload, map<string|string[]> headers = {}) returns error? {
        oas:Client oasClient = self.getOasClient();
        error? r = oasClient->executeLibraryFunction(formulaName, elementName, functionName, payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
    ...
    # + headers - Headers to be sent with the request 
    # + return - OK 
    remote isolated function massEditImports(string typedId, oas:ImportmanagerMassedittypedIdBody payload, map<string|string[]> headers = {}) returns error? {
        oas:Client oasClient = self.getOasClient();
        error? r = oasClient->massEditImports(typedId, payload, headers);
        if isAuthError(r) {
            check self.reauthenticate();
            oasClient = self.getOasClient();
