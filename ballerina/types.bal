...
// >>> MANUALLY MAINTAINED - NOT REGENERATED. `PricefxCredentials` and `ConnectionConfig` are
// hand-written overrides (see docs/spec/sanitations.md item 571); `bal openapi` does not produce
// them. Reconcile this block against any regenerated output before committing a regeneration. <<<
...
public type PricefxCredentials record {|
    ...
    string password;
    # The partition to authenticate against. Required when `pricefxKey` is set
    string partition?;
    ...
    # absent, authentication falls back to `GET /login/extended` (HTTP Basic auth), which needs
    # no separate API key but is slower per request
    string pricefxKey?;
|};
...
public type ConnectionConfig record {|
    # Pricefx account credentials, exchanged internally for a short-lived JWT. The client
    # automatically re-authenticates and retries once whenever a request comes back
    # unauthenticated, so a long-lived client instance keeps working without manual
    # re-initialization
...
|};
// <<< END MANUALLY MAINTAINED block (PricefxCredentials / ConnectionConfig) >>>
